...
1. Vor dem ersten Datenimport müssen Sie am Vitalcontrol einmal Ihre [Betriebsnummer eingeben]({{< relref "../settings/farm-number.md" >}}).
...
2. Verbinden Sie das VitalControl-Gerät via des mitgelieferten USB-Kabels mit ihrem Computer oder Laptop.
   ...
   ![VitalControl mit dem PC verbinden](/images/firmware/update/plug-in-dual-usb-stick.svg "VitalControl mit PC koppeln")

3. Stellen Sie sicher, dass das VitalControl-Gerät eingeschaltet ist.
...
1. Es öffnet sich ein DOS-Fenster, der Importvorgang wird gestartet. Es werden mehrere Abfragen gestellt, anhand derer sie Angaben zum Umfang des Tierimports festlegen können. Hierbei werden abgefragt:
   ...
   - ➀ **Geschlecht** der zu importierenden Tiere:  
     Drücken Sie hier `M` oder `W` um nur **m**ännliche bzw. **w**eibliche Tiere zu importieren oder drücken Sie `OK` um alle Tiere anzurufen)
...
1. Das Programm stellt nun anhand der im VitalControl hinterlegten [Betriebsnummer eingeben]({{< relref "../settings/farm-number.md" >}}) die Verbindung zur HI-Tier Datenbank her. In diesem Zuge wird ihr ➄ **Passwort** bei HI-Tier abgefragt, tippen Sie dieses im Fenster ein.
...
1. Der Import der Tiere ist nach einer gewissen Zeit geschlossen. Wenn Sie im Hauptbildschirm Ihres VitalControl-Geräts den Menüpunkt `Herde` aufrufen, können Sie sich davon überzeugen, ob alle Tiere erfolgreich übertragen wurden.
