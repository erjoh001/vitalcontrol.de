...
2. A submenu opens in which various settings are displayed. Use the arrow keys △ ▽ to select `Dieren registreren`. Confirm with `OK`.

3. Another submenu opens in which you can use the arrow keys △ ▽ to switch between the setting fields for `Standaard waardes`, `Dieren automatisch registreren` and `Toevoegen Oormerk`. Select `Standaard waardes` and confirm with `OK`.

4. The `Opgenomen gewicht` area is automatically selected. Use the arrow keys ◁ ▷ to select the weight to be captured.
...
4. Use the arrow keys △ ▽ to select the `Leeftijd nieuw kalf/lam`.
...
4. Use the arrow keys △ ▽ to select the `Geslacht`.
...
4. Use the arrow keys △ ▽ to select the `Type van nutdier`.
...
4. Use the arrow keys △ ▽ to select `Ras`.
...
4. Use the arrow keys △ ▽ to select `verblijfplaats`.
...
3. Another submenu opens in which you can use the arrow keys △ ▽ to switch between the setting fields for `Standaard waardes`, `Dieren automatisch registreren` and `Toevoegen Oormerk`. Select `Dieren automatisch registreren` and confirm with `OK`.

4. The preference field `Dieren automatisch registreren` is selected automatically. Use the arrow keys ◁ ▷ to determine the behavior of the device when creating new animals.

    ![VitalControl: menu settings auto-registration](../images/autoregistration.png "auto-registration")

## Toevoegen Diernummer

Here you define which animal ID is proposed (Afleiding van het treanspondernummer) or used (opeenvolgend) when creating a new animal in your farm.

{{% alert title = "Hint" %}}
Select the "opeenvolgend" option if the stall number you use for visual identification of the animal is assigned independently of the transponder number used for electronic identification. The stall or collar numbers are then assigned consecutively within a certain range (e.g. 1 - 40) and are then not directly related to the transponder numbers used. Select the option "Derive from transponder number" if you want to link the animal's stall number directly to the number of the transponder that was assigned to the animal. This option is especially useful if you use yellow electronic ear tags, the barn number is then identical to the end digits of the ten-digit animal number printed on the yellow ear tag.
{{% /alert %}}
...
{{% alert title = "Hint" %}}
Only when using the option `opeenvolgend`: if you have set to use separate number ranges for male and female animals when assigning the ear tag number, a next animal number is stored for each of male, female and animals of unknown sex. A symbol placed to the left of the animal number indicates what the next animal number is used for: either for male ♀ or for female ♂ or for animals of unknown sex ♀♂?
{{% /alert %}}
...
4. Use the arrow keys △ ▽ to select `Cijfer van nieuw ID`.
...
4. Use the arrow keys △ ▽ to select `Offset rechts`.
...
3. Another submenu opens in which you can use the arrow keys △ ▽ to switch between the setting fields for `Standaard waardes`, `Dieren automatisch registreren` and `Toevoegen Oormerk`. Select `Toevoegen Oormerk` and confirm with `OK`.
...
Save the settings and return to the top menu `Toevoegen Oormerk` by pressing the `F1` key &nbsp;<img src="/icons/footer/save_exit.svg" width="65" align="bottom" alt="Save and return" />&nbsp;.
