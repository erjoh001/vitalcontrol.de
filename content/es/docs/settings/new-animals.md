...
linkTitle: "Registro de animales"
...
1. En la pantalla principal de tu dispositivo VitalControl, selecciona el ítem del menú <img src="/icons/gear.svg" width="25" align="bottom" alt="Settings" /> `Configuración` y presiona la tecla `OK`.

2. Se abre un submenú en el que se muestran varias configuraciones. Usa las teclas de flecha △ ▽ para seleccionar `Registro de animales`. Confirma con `OK`.

3. Se abre otro submenú en el que puedes usar las teclas de flecha △ ▽ para alternar entre los campos de configuración para `Valores predeterminados`, `Registro automático` y `Asignación de número de etiqueta de oreja`. Selecciona `Valores predeterminados` y confirma con `OK`.
...
1. En la pantalla principal de su dispositivo VitalControl, seleccione el elemento del menú <img src="/icons/gear.svg" width="25" align="bottom" alt="Settings" /> `Configuración` y presione la tecla `OK`.

2. Se abre un submenú en el que se muestran varios ajustes. Utilice las teclas de flecha △ ▽ para seleccionar `Registro de animales`. Confirme con `OK`.

3. Se abre otro submenú en el que puede usar las teclas de flecha △ ▽ para alternar entre los campos de configuración para `Valores predeterminados`, `Registro automático` y `Asignación de número de etiqueta de oreja`. Seleccione `Valores predeterminados` y confirme con `OK`.

4. Utilice las teclas de flecha △ ▽ para seleccionar la `Edad al registro`.
...
2. Se abre un submenú en el que se muestran varios ajustes. Utiliza las flechas △ ▽ para seleccionar `Registro de animales`. Confirma con `OK`.

3. Se abre otro submenú en el que puedes usar las flechas △ ▽ para alternar entre los campos de ajuste para `Valores predeterminados`, `Registro automático` y `Asignación de número de arete`. Selecciona `Valores predeterminados` y confirma con `OK`.

4. Utiliza las flechas △ ▽ para seleccionar el `Tipo de animal`.
...
3. Se abre otro submenú en el que puedes usar las teclas de flecha △ ▽ para cambiar entre los campos de configuración para `Valores predeterminados`, `Auto-registro` y `Número de etiqueta de oreja`. Selecciona `Valores predeterminados` y confirma con `OK`.

4. Usa las teclas de flecha △ ▽ para seleccionar `Paradero`.
...
2. Se abre un submenú en el que se muestran varios ajustes. Usa las teclas de flecha △ ▽ para seleccionar `Registro de animales`. Confirma con `OK`.

3. Se abre otro submenú en el que puedes usar las teclas de flecha △ ▽ para cambiar entre los campos de configuración para `Valores predeterminados`, `Auto-registro` y `Número de etiqueta de oreja`. Selecciona `Auto-registro` y confirma con `OK`.

4. El campo de preferencia `Auto-registro` se selecciona automáticamente. Usa las teclas de flecha ◁ ▷ para determinar el comportamiento del dispositivo al crear nuevos animales.

![VitalControl: configuración del menú auto-registro](../images/autoregistration.png "auto-registro")

## Asignación de ID de animal

Aquí defines qué ID de animal se propone (derivada del número de transpondedor) o se utiliza (consecutiva) al crear un nuevo animal en tu granja.

{{% alert title = "Consejo" %}}
Selecciona la opción "Consecutiva" si el número de establo que usas para la identificación visual del animal se asigna independientemente del número de transpondedor utilizado para la identificación electrónica. Los números de establo o collar se asignan entonces consecutivamente dentro de un cierto rango (por ejemplo, 1 - 40) y luego no están directamente relacionados con los números de transpondedor utilizados. Selecciona la opción "Derivar del número de transpondedor" si deseas vincular el número de establo del animal directamente al número del transpondedor que se le asignó al animal. Esta opción es especialmente útil si usas etiquetas electrónicas amarillas para las orejas; el número del establo será entonces idéntico a los últimos dígitos del número de animal de diez dígitos impreso en la etiqueta amarilla.
{{% /alert %}}
...
3. Se abre otro submenú en el que puedes usar las teclas de flecha △ ▽ para alternar entre los campos de configuración para `Valores predeterminados`, `Auto-registro` y `Asignación número de etiqueta de oreja`. Selecciona `Auto-registro` y confirma con `OK`.
...
1. En la pantalla principal de tu dispositivo VitalControl, selecciona el ítem del menú <img src="/icons/gear.svg" width="25" align="bottom" alt="Configuración" /> `Configuración` y presiona la tecla `OK`.
...
3. Se abre otro submenú en el que puede usar las teclas de flecha △ ▽ para alternar entre los campos de ajuste para `Valores predeterminados`, `Auto-registro` y `Asignación número de etiqueta de oreja`. Seleccione `Auto-registro` y confirme con `OK`.

4. Utilice las teclas de flecha △ ▽ para seleccionar `Desplazamiento derecho`.
...
## Asignación número de etiqueta de oreja
...
2. Se abre un submenú en el que se muestran varios ajustes. Utiliza las teclas de flecha △ ▽ para seleccionar `Registro de animales`. Confirma con `OK`.

3. Se abre otro submenú en el que puedes usar las teclas de flecha △ ▽ para alternar entre los campos de ajuste para `Valores predeterminados`, `Auto-registro` y `Asignación número de arete`. Selecciona `Asignación número de arete` y confirma con `OK`.

4. Utiliza las teclas de flecha ◁ ▷ para asignar el número de arete al animal de trabajo al crear.

    ![VitalControl: ajustes del menú Asignación número de arete](../images/assignmenteartagnumber.png "Asignación número de arete")

Guarda los ajustes y regresa al menú principal `Registro de animales` presionando la tecla `F1` &nbsp;<img src="/icons/footer/save_exit.svg" width="65" align="bottom" alt="Guardar y regresar" />&nbsp;.
